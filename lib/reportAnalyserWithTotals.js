...
async function analyseReportWithTotals(csvPath, accountIds, groupBy, internalGroupBy, showTotalBy, priceColumns) {
    try {
        ...
        if (!priceColumns || priceColumns.length === 0) {
            ...
            console.log(`DEBUG: Auto-detected price columns:`, priceColumns);
        }
        ...
        if (accountIds && accountIds.length > 0) {
            console.log(`DEBUG: Filtering for account IDs:`, accountIds);
            const allAccountIds = [...new Set(records.map(r => r.account_id))];
            console.log(`DEBUG: Available account IDs in CSV:`, allAccountIds);
            
            filteredRecords = records.filter(record =>
                accountIds.includes(record.account_id)
            );
            
            console.log(`DEBUG: Records after filtering: ${filteredRecords.length} (from ${records.length} total)`);
        ...
        return {
            ...
            currency,
            priceColumns, // Include priceColumns in the response for the EJS template
            metadata: {
...
function autoDetectPriceColumns(records, columns) {
    ...
    columns.forEach(col => {
        ...
        // Check if column name suggests it's a price/money column
...
function processAccountGroup(accountGroup, groupBy, internalGroupBy, showTotalBy, priceColumns, currency) {
    ...
    console.log(`DEBUG: Created ${Object.keys(mainGroups).length} groups by '${groupBy}':`);
